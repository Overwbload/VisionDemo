<template>
  <div class="position-relative d-flex">
        ...
        <div class="col-md-8 d-flex flex-column">
          <div class="my-auto text-center">
            ...
            <div>{{ $t('checkoutSuccess.emailMessage') }} <span class="fs-20">{{ order.orderId }}</span>
              <button @click="copyOrderId" class="btn btn-sm btn-outline-dark ms-2">{{ $t('checkoutSuccess.copy') }}</button>
            </div>
    ...
    background-size: cover; background-position: center center; opacity: 0.8;">
      <video autoplay loop muted playsinline title="" class="w-100 h-100 object-fit-cover">
